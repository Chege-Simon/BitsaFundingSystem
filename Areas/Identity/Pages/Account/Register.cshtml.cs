...
namespace BitsaFundingSystem.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var inputBytes = Encoding.ASCII.GetBytes($"{Input.First_Name}-{Input.First_Name}-{Input.First_Name ?? ""}-{DateTime.Now}");
                var outputBytes = sha256.ComputeHash(inputBytes);
                //string id = outputBytes.ToString();
                string id = Convert.ToBase64String(outputBytes);
                var user = new BitsaFundingSystemUser
                {
